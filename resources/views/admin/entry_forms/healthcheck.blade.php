...
<body>
    ...
    <p style="font-size:large; text-align:center;">第{{ config('app.number_of_times') }}回東京連盟ローバースカウト100kmハイク<br>
        健康調査票 兼 Eラーニング修了証</p>
    <p class="">ゼッケン: {{ $entryForm->zekken }}</p>

    <table>
        <tbody>
            <tr>
                <td>基本情報</td>
                <td>{{ $entryForm->name }}({{ $entryForm->furigana }})
                    【性別:】{{ $entryForm->gender }}</td>
            </tr>
            <tr>
                <td>所属</td>
                <td>{{ $entryForm->prefecture }}連盟 {{ $entryForm->district }}地区
                    {{ $entryForm->dan_name }}団 【登録番号:】 {{ $entryForm->bs_id }}
                </td>
            </tr>
            <tr>
                <td>生年月日</td>
                <td>{{ $entryForm->birth_day->format('Y年m月d日') }}
                    ({{ \Carbon\Carbon::parse($entryForm->birth_day)->age }}才)</td>
            </tr>
            <tr>
                <td>住所</td>
            ...
            <tr>
                <td>本人連絡先</td>
                <td>【自宅:】 {{ $entryForm->telephone }} 【ケータイ:】 {{ $entryForm->cellphone }}<br>
            ...
            <tr>
                ...
                <td>{{ $entryForm->parent_name }}
                    ({{ $entryForm->parent_name_furigana }})
                    【続柄:】{{ $entryForm->parent_relation }}<br>
                    【緊急連絡先:】 {{ $entryForm->emer_phone }}
                </td>
            </tr>
        </tbody>
    </table>
